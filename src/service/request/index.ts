  ...
  },
  {
    ...
    isBackendSuccess(response) {
      ...
      if (import.meta.env.VITE_APP_ENCRYPT === 'Y' && response.headers[encryptHeader]) {
        ...
        const base64Str = decryptByRsa(keyStr, import.meta.env.VITE_APP_RSA_PRIVATE_KEY || '');
        if (!base64Str) {
          throw new Error('RSA decrypt failed');
        }
        const aesKey = decryptBase64(base64Str as string);
...
function handleEncrypt(config: InternalAxiosRequestConfig) {
  ...
  if (import.meta.env.VITE_APP_ENCRYPT === 'Y') {
    ...
    if (isEncrypt && (config.method === 'post' || config.method === 'put')) {
      ...
      const encryptedAesKey = encryptByRsa(encryptBase64(aesKey), import.meta.env.VITE_APP_RSA_PUBLIC_KEY || '');
      if (!encryptedAesKey) {
        throw new Error('RSA encrypt failed');
      }
      config.headers[encryptHeader] = encryptedAesKey;
      config.data =
