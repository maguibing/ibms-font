...
function getValidationMessage() {
  if (!actionGroups.value.length) return '请至少添加一个执行设备';
  for (const [index, action] of actionGroups.value.entries()) {
    if (!action.device_id) return `请选择执行设备 ${index + 1} 的设备`;
    ...
    if (inKeys.some(key => key.endsWith('-'))) return `请选择执行设备 ${index + 1} 的点位`;
    if (new Set(inKeys).size !== inKeys.length) return `执行设备 ${index + 1} 存在重复点位`;
    if (inKeys.sort().join('|') !== outKeys.sort().join('|')) return `执行设备 ${index + 1} 的范围内外点位必须一致`;
    for (const [pointIndex, point] of action.point_vals.entries()) {
      if (!point.device_type_point_id) return `请选择执行设备 ${index + 1} 的点位 ${pointIndex + 1}`;
      if (
        !point.data_type ||
