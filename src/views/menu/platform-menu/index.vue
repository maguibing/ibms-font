...
import PlatformMenuOperateDrawer from './modules/platform-menu-operate-drawer.vue';
...
async function getMenuTree(selectId?: CommonType.IdType) {
  startLoading();
  const { data, error } = await fetchGetPlatformMenuTrees({ p_type: PLATFORM_P_TYPE, menu_type_list: [1,2,3,4]});
  endLoading();
  if (error) return;
