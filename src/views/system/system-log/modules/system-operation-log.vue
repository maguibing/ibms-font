...
import { fetchGetSystemLogList } from '@/service/api/system-log';
...
function getOperatorName(userId?: CommonType.IdType) {
  ...
  const raw = extraData.value as Api.System.SystemOperationLogListExtra | null;

  return raw?.base_user_map?.[String(userId)]?.username ?? String(userId);
}

function createColumns(): NaiveUI.TableColumn<Api.System.SystemOperationLog>[] {
  return [
    {
      key: 'index',
...
<template>
  <div class="min-h-880px flex-col-stretch gap-16px overflow-hidden lt-lg:overflow-auto">
